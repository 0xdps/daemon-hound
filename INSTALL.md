...
DaemonHound ships as a single `dh` binary plus optional package-manager artifacts from GitHub Releases.

## Linux

### Install Script

This is the quickest path on most Linux machines. It detects your OS and CPU architecture, downloads the latest GitHub Release archive, and installs `dh` to `/usr/local/bin` by default.
...
Download the Windows `.zip` asset from the latest release, extract it, and place `dh.exe` somewhere in `PATH`.
...
`dh init` installs a user-level background daemon where supported:
